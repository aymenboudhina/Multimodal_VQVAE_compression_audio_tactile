...
Goal:
  ...
  - Packet loss is sampled in three categories (low/medium/high) based on
    maximum burst length in milliseconds, inspired by PLC Challenge:
      * low   : up to ~120 ms
      * medium: 120–320 ms
      * high  : 320–1000 ms
    with sampling probabilities 0.52 / 0.32 / 0.16.
...
# We approximate the PLC Challenge categories (max burst length in ms):
#   low   : up to 120 ms
#   medium: 120–320 ms
#   high  : 320–1000 ms
# and use the same proportions: 52% / 32% / 16% of data.
CAT_PROBS = {
    "low":   0.52,
    "medium":0.32,
    "high":  0.16,
...
CAT_BURST_MS = {
    "low":   (20.0, 120.0),   # min 20 ms, max 120 ms
...
# ================== CATEGORY-BASED PACKET LOSS ==================
def sample_category() -> str:
    """Sample a loss category according to CAT_PROBS."""
    r = random.random()
    if r < CAT_PROBS["low"]:
        return "low"
    elif r < CAT_PROBS["low"] + CAT_PROBS["medium"]:
        return "medium"
    else:
        return "high"

def make_category_token_loss_mask(
    batch_size: int,
    T_lat: int,
    ...
    """
    Simulate packet loss on latent tokens with three categories:
      - low   : max burst  ~20–120 ms
      - medium: max burst  ~120–320 ms
      - high  : max burst  ~320–1000 ms

    For each sample in the batch:
      * Choose a category according to CAT_PROBS.
      * Draw a number of bursts from CAT_N_BURSTS[cat].
      * For each burst, draw a length in ms from CAT_BURST_MS[cat],
        convert to tokens using tokens_per_sec, and set those tokens
        to "lost" (True) in the mask.
    ...
    for b in range(batch_size):
        cat = sample_category()
        min_ms, max_ms = CAT_BURST_MS[cat]
        nb_min, nb_max = CAT_N_BURSTS[cat]
...
class AllPredPLC(nn.Module):
    ...
    Audio→Tactile packet-loss concealment model:
      ...
      - Randomly mask (drop) tactile latent tokens using category-based
        burst lengths (low/medium/high).
    ...
    def forward_step(self, a_1T, tc_1T):
        ...
        qa, *_ = self.A_QUANT(za)    # [B,C,T_lat_a] quantized audio latents

        # Tactile backbone
        zt_full = self.T_ENC(tc_1T)  # [B,C,T_lat_t]
        B, C, T_lat = zt_full.shape

        # We assume SEG_SEC = 1.0 s, so tokens_per_sec ≈ T_lat
        tokens_per_sec = T_lat / float(SEG_SEC) if SEG_SEC > 0 else float(T_lat)

        # Create category-based burst loss mask on latent tokens
        mask_tokens = make_category_token_loss_mask(
            batch_size=B,
            T_lat=T_lat,
        ...
        # If audio and tactile latents differ in T, interpolate audio to tactile length
        if qa.shape[-1] != T_lat:
            qa_res = F.interpolate(
                qa, size=T_lat, mode="linear", align_corners=False
            )
        else:
            qa_res = qa
        ...
        torch.save(
            {
                ...
                "category_burst_ms": CAT_BURST_MS,
                "category_n_bursts": CAT_N_BURSTS,
            },
