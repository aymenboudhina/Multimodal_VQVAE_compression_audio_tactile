...
#   pip install scipy soundfile h5py
...
OUTPUT_DIR  = r"C:\Users\Aymen\Master thesis\WAV_Files"               # <-- change if needed
...
def _extract_from_finalMaterialRecording(fmr):
    """
    Try common ways to access 'sound' field inside finalMaterialRecording.
...
def _first_1d_numeric_named(mat_dict, name_hint="sound"):
    """
...
def extract_sound_signal(mat_obj, backend: str):
    """
    Try to extract the 'sound' 1D signal from the loaded MAT content.
    Returns np.ndarray (float32) or None.
    """
    if backend == 'scipy':
        mat = mat_obj  # dict-like
        # 1) Preferred: finalMaterialRecording.sound
        ...
        # 2) Fallback: top-level 'sound' key
        if 'sound' in mat:
            return _ensure_1d_mono(mat['sound'])

        # 3) Last resort: scan for anything named like 'sound'
        snd = _first_1d_numeric_named(mat, name_hint="sound")
        if snd is not None:
            return _ensure_1d_mono(snd)
        ...
        # Broad search for datasets containing "sound"
...
def process_mat_file(mat_path: str, wav_output_dir: str, sample_rate: int = SAMPLE_RATE) -> bool:
    """
    Try to load and extract 'sound' from .mat.
    ...
    try:
        sound = extract_sound_signal(mat_obj, backend)
    finally:
    ...
    if sound is None or sound.size == 0:
        print(f"⏩  Skipping (no 'sound' found): {mat_path}")
        return False
    ...
    save_wav(sound, wav_path, sample_rate)
...
def main():
    ...
    print(f"Skipped (no sound): {skipped}")
