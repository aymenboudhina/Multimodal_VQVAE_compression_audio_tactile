...
# -*- coding: utf-8 -*-
"""
Train — Low-bitrate AR A->T with RVQ (safe autograd: no in-place clamps)
------------------------------------------------------------------------
- Cross-predict tactile latents from (prev z_hat, audio quantized tokens).
- Residual RVQ is tiny (few books) to keep bitrate low.
- Loss: L1 (wave), MR-STFT, Mel-Cosine (structure), optional latent guide.
- All normalization uses non-inplace ops (no .clamp_min_ / .clamp_max_).
...
OUT_DIR    = r"/home/student/studentdata/Train_ALLPRED_AR_LowBR_Safe"
...
AR_CHUNK_TOK   = 16          # tokens per AR block

# Residual VQ (LOW BITRATE)
CODE_DIM       = 96          # slightly smaller code dim
RVQ_N_BOOKS    = 10           # keep low; raise to 4 if needed
RVQ_EMBED      = 128       # 7 bits per book → 3 books ≈ 21 bits/token
EMA_DECAY      = 0.99
EMA_WARM_E     = 5
...
W_STFT         = 0.25
W_MELCOS       = 0.20
W_LAT          = 0.03        # small latent guidance; set 0.0 to disable

# Mel/STFT config for loss & metric
MEL_NFFT = 512
MEL_HOP  = 128
MEL_MELS = 64
EPS      = 1e-7
...
def resample_to(wav, sr_in, sr_out):
    ...
    # Always do resample in float32, AMP disabled
...
class MultiResSTFTLoss(nn.Module):
    ...
    def _stft_mag(x, n_fft, hop, win, eps):
        ...
        return spec.abs().clamp_min(eps)   # functional (safe)
...
class MelCosineLoss(nn.Module):
    """
    Cosine distance on normalized log-mel magnitudes (structure-focused).
    """
    ...
    def _mel_mag(self, x_1T: torch.Tensor):
        ...
        mag = spec.abs().clamp_min(self.eps)              # functional clamp
        M = self.mel.to(x.device)(mag)
        den = M.amax(dim=(1,2), keepdim=True).clamp_min(self.eps)  # no in-place
        M = M / den
        M = (M + self.eps).log()
        return M
    def forward(self, x, y):
        X = self._mel_mag(x); Y = self._mel_mag(y)
        # time-align by linear resize (safe)
...
class ResidualVQEMA(nn.Module):
    def __init__(self, dim=CODE_DIM, n_books=RVQ_N_BOOKS, n_embed=RVQ_EMBED, decay=EMA_DECAY):
        super().__init__()
    ...
    def forward(self, z):
        """z:[B,D,T] -> q_sum:[B,D,T] (train uses all books)."""
...
class AllPredAR(nn.Module):
    """
    Audio: encoder + DAC quantizer (qa).
    Tactile: encoder (teacher latent only for guide), AR over chunks:
      z_pred = Predict(z_prev, qa_chunk)
      rD     = proj_down(scale * tanh(TokenNorm(zt - stopgrad(z_pred))))
      qD     = RVQ(rD)
      z_hat  = z_pred + proj_up(qD)
      z_prev for next token comes from z_hat (shifted).
    """
    def __init__(self, A_ENC, A_QUANT, T_ENC, T_DEC, c_lat):
        ...
        self.vq        = ResidualVQEMA(dim=CODE_DIM, n_books=RVQ_N_BOOKS, n_embed=RVQ_EMBED, decay=EMA_DECAY)

    def forward_step(self, a_1T, tc_1T):
        B,_,Tw = tc_1T.shape
        za = self.A_ENC(a_1T)
        qa, *_ = self.A_QUANT(za)
        zt_teacher = self.T_ENC(tc_1T)        # only used for residual target + latent guide
        ...
        for s in range(0, Tlat, AR_CHUNK_TOK):
            ...
            scale = self.scale.clamp(5e-3, 0.5)           # functional clamp
        ...
        return {
            "y_hat":  finite_or_zero(y_hat[...,:T]),
            "tgt":    finite_or_zero(tc_1T[...,:T]),
            "z_pred": torch.cat([self.predict(
                        torch.zeros_like(zt_teacher[..., :min(AR_CHUNK_TOK,Tlat)]),  # dummy for shape
                        qa[..., :min(AR_CHUNK_TOK,Tlat)])], dim=-1) if Tlat>0 else None,  # (not used directly)
            "z_teacher": zt_teacher,
...
def train():
    ...
    net = AllPredAR(A_ENC, A_QUANT, T_ENC, T_DEC, c_lat=C).to(DEVICE)
    ...
    best_val = float("inf")
    hist = {"train":[], "val":[], "l1":[], "stft":[], "mel":[], "lat":[], "ema":[]}

    def step(a, tc, train_mode=True, epoch=1) -> Tuple[float,float,float,float,float]:
        ...
        with autocast('cuda', enabled=USE_AMP):
            ...
            llat = 0.0
            if W_LAT > 0.0 and out["z_teacher"] is not None:
                # weak guidance: align predicted features with teacher latents (shape-safe slice)
                zt = out["z_teacher"].detach()
                # use z_run via decoder-encoder roundtrip for strictness? Keep simple: none
                # Here, guide with waveform features instead (already via MELCOS/STFT). Keep latent small:
                llat = 0.0

            total = W_WAV_L1*l1 + W_STFT*lstf + W_MELCOS*lmel + W_LAT*llat
    ...
    for ep in range(1, EPOCHS+1):
        ...
        print(f"Epoch {ep:03d} | train {t_avg:.4f} | val {v:.4f} | "
    ...
    plt.title("ALL-PRED AR A->T — Low Bitrate (safe autograd)")
    plt.tight_layout(); plt.savefig(os.path.join(OUT_DIR, "curves.png")); plt.close()

    with open(os.path.join(OUT_DIR,"hist.json"),"w") as f:
